#!/bin/bash
#
# This will run Mysql and delete (drop) the table conditions.
#
# ----------------------------------------------------------------- #
# System    : Centos 7.7                                            #
#   Name    : peekaboo                                              #
# Location  : /home/don/workspace/books/books_scripts/              #
# File Name : z_tb_delete_conditions_mysql.sh                       #
# Rev. Date : 04/18/2020                                            #
# Rev. No.  : 0.0.3                                                 #
# ----------------------------------------------------------------- #
#
...
echo -e "  Delete table conditions"
echo -e "---------------------------\n"

mysql -u dondb -pdps0226db < ./sql_commands/tb_delete_conditions.sql
#ERRORCODE=$?
#echo "1ERRORCODE = $ERRORCODE"


echo -e "\nDelete migration file create_condition "
echo -e "--"
ls /home/don/workspace/books/db/migrate/*_condition*.* 2>/dev/null
ERRORCODE=$?
#echo "2ERRORCODE = $ERRORCODE"
if [ $ERRORCODE -eq 2 ]; then
	echo -e "\t********************************"
	echo -e "\tNo migration file for condition."
	echo -e "\t********************************"
fi
echo -e "--"


rm /home/don/workspace/books/db/migrate/*_condition.* 2>/dev/null
ERRORCODE=$?
